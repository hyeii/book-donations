...
public interface JpaHistoryRepository extends JpaRepository<HistoryEntity, Long> {
    ...
    @Query(value = "SELECT h " +
            "FROM HistoryEntity h " +
            "WHERE h.memberId = :memberId AND h.status = com.bookdone.history.application.HistoryStatus.UNWRITTEN " +
            "ORDER BY h.donatedAt DESC")
    List<HistoryEntity> findAllByMemberIdAndStatusIsUnwritten(Long memberId);
